...
int cal(int i, int prev, int first) {
    if (i == n+1) {
        return 0;
    }
    if (dp[i][prev][first] != -1) {
        return dp[i][prev][first];
    }    
    int ans = cal(i+1, 0, first);

    if (i == n) {
        if (!prev && !first) {
            ans = max(ans, arr[i] + cal(i+1, 1, first));
        }
    }
    else {
        if (!prev) {
