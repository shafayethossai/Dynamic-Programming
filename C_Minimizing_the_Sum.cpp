...
ll rec(ll i, ll left) {
    ...
    ll ans = v[i] + rec(i+1, left);
    
    for (int j=1; j<=left && i+j < n; j++) { // gropuping with 1, 2, .., jth
        ll min1 = v[i];

        for (int k=i+1; k<=i+j; k++) { // min (i to i+j)
            min1 = min(min1, v[k]);
        }
        ans = min(ans, min1 * (j+1) + rec(i+j+1, left - j)); // min*len + next
    }
